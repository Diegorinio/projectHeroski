...
public class playerTileSecondDetector : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if(!gameObject.GetComponentInParent<playerCharacter>().targetEnemy){
            Debug.Log($"second detector: {collision.gameObject.tag}");
            enemyMarked = collision.gameObject;
            gameObject.GetComponentInParent<playerCharacter>().targetEnemy = collision.gameObject;
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Enemy"))
        {
            ...
            gameObject.GetComponentInParent<playerCharacter>().targetEnemy = null;
        }
    }
